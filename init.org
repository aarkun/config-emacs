...
Variables or faces can be set by [[https://www.gnu.org/software/emacs/manual/html_node/elisp/Customization.html][Emacs Customize interface]]. Customization shall be set explicitly in this file. Therefore, the following code blocks should always be empty:
...
** Use use-package
Install [[https://www.gnu.org/software/emacs/manual/html_node/use-package/index.html][use-package]], if not done yet:
#+begin_src emacs-lisp
...
1. Ensure newly installed packages are downloaded and installed properly before running.
2. Install [[https://elpa.gnu.org/packages/gnu-elpa-keyring-update.html][gnu-elpa-keyring-update]] to keep GPG keyring or GNU ELPA up to date and keep the default behavior.
#+begin_src emacs-lisp
...
* Enable Diminish
To clean up the mode-line, [[https://github.com/emacsmirror/diminish][diminish]] is installed and can be used for certain minor modes. The Installation is done, like described in the [[https://www.gnu.org/software/emacs/manual/html_node/use-package/Diminish.html][Emacs manual]]:
#+begin_src emacs-lisp
  (use-package diminish)
#+end_src

* Hide built-in minor modes in the mode-line
The [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Visual-Line-Mode.html][Visual Line mode]] is a built-in minor modes, which shall be hidden in the mode-line, because it offers no functionality. As built-in modes it is diminished using the package ~emacs~:
#+begin_src emacs-lisp
  (use-package emacs
    :diminish visual-line-mode)
#+end_src

* Customize Auto Revert
The [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Auto-Revert.html][Auto Revert mode]] shall be hidden in the mode-line, because they offers no functionality:
#+begin_src emacs-lisp
...
* Use which-key
Install and activate [[https://github.com/justbur/emacs-which-key][which-key]] minor mode to display current awailable key bindings while typing. This minor mode is hidden in the mode-line, because it offers no functionality:
#+begin_src emacs-lisp
...
Install [[https://github.com/Yevgnen/ivy-rich][ivy-rich]] and activate it, to have more actions with /M-o/ and better UI in the minibuffer for Ivy and Counsel completions. This must be loaded, after Ivy was loaded:
#+begin_src emacs-lisp
...
* Use Projectile
Install [[https://github.com/bbatsov/projectile][Projectile]] and set the key prefix for Projectile commands to /C-c p/. All commands are bound to keys, so the entry in the mode-line can be hidden:
#+begin_src emacs-lisp
...
* Use Magit
Install [[https://magit.vc/][Magit]] to have a nice Git integration. Also enable Visual line mode for Magit status mode, to have line wraps for long lines as a default:
#+begin_src emacs-lisp
  (use-package magit
...
Flycheck is not needed in ~org-mode~. Source code snippets in Org mode should be edited in a separate buffer, in which the specific major mode with Flycheck is enabled again.
...
** Customize JavaScript major mode
Use 2 spaces for indentation of JavaScript and JSON files:
#+begin_src emacs-lisp
  (use-package js
...
** Use Org mode
Several adjustments are done for Org mode:

*** Define agenda files
The list of agenda files is specific to different environments and user accounts. Each user account has to define its own list of files in a textfile ~org-agenda-files~ and put this in the user-specific emacs configuration directory (default: =$HOME/.emacs.d=). The file =org-agenda-files= should look  like the following example:
#+begin_example
...
...
#+end_example
If the file does not exist, it is created empty during startup.

*** Add evaluation of shell codeblocks
To enable evaluation of ~shell~ codeblocks within Org files, like described in the [[https://orgmode.org/guide/Working-with-Source-Code.html#Evaluating-code-blocks-1][Org manual]], they have to be added to =org-babel-load-languages=.

*** Add evaluation of Python codeblocks
To enable evaluation of ~python~ codeblocks within Org files, like described in the [[https://orgmode.org/guide/Working-with-Source-Code.html#Evaluating-code-blocks-1][Org manual]], they have to be added to =org-babel-load-languages=.

*** Enable Visual Line mode
Since Org mode is for editing texts, Visual Line mode with word wrap is enabled.

*** Flyspell mode
Not all Org files need spell checking. So Flyspell mode should be activated specific for each Org files, by adding one of the following ~eval~ expressions as the first list:
#+begin_src org
  # -*- eval: (flyspell-mode) -*-
#+end_src

Or, if source is included:
#+begin_src org
  # -*- eval: (flyspell-prog-mode) -*-
#+end_src

To set a specific dictionary, the following line can be added as a comment:
#+begin_src org
  # Local IspellDict: en
#+end_src
...
** Use json-mode
To have better syntax highlighting for JSON files, install [[https://github.com/json-emacs/json-mode][json-mode]]:
#+begin_src emacs-lisp
...
** Use Robot
Install [[https://github.com/kopoli/robot-mode][robot-mode.el]]:
#+begin_src emacs-lisp
  (use-package robot-mode)
#+end_src

** Use Pandoc-mode
Add minor mode [[https://joostkremers.github.io/pandoc-mode/][Pandoc-mode]] to interact with [[https://pandoc.org][Pandoc]] from within Emacs:
#+begin_src emacs-lisp
...
** Use go-template-mode
Install [[https://codeberg.org/rch/go-template-mode][go-template-mode]]:
#+begin_src emacs-lisp
  (use-package go-template-mode)
#+end_src

** Use go-template-helper-mode
Install [[https://codeberg.org/rch/go-template-helper-mode][go-template-helper-mode]] for editing Helm charts, which means, the file is located in a directory =templates/= and ends with =.yaml=:
#+begin_src emacs-lisp
...
*Note*: It is necessary, that  set up and that the command ~npm i -g <language-server>~ can be executed!
#+begin_src emacs-lisp
...
** Language support
*** Bash
[[https://emacs-lsp.github.io/lsp-mode/page/lsp-bash/][Bash LSP support]] requires an installation of https://github.com/bash-lsp/bash-language-server, which is done by registering the ~lsp~ command to the ~sh-mode~ in a hook. The installation requires an installation of [[https://nodejs.org/][Node.js]] in version 20 or greater or equals than version 22.
...
*** YAML
[[https://emacs-lsp.github.io/lsp-mode/page/lsp-yaml/][YAML LSP support]] requires an installation of https://github.com/redhat-developer/yaml-language-server, which is done by registering the ~lsp~ command to the ~yaml-mode~ in a hook.

*** Lua
[[https://emacs-lsp.github.io/lsp-mode/page/lsp-lua-language-server/][Lua LSP support]] requires an installation of https://github.com/LuaLS/lua-language-server, which is done by registering the ~lsp~ command to the [[* Use Lua-mode][Lua-mode]] in a hook.
