...
** Use mermaid-mode
Install [[https://github.com/abrochard/mermaid-mode][mermaid-mode]] and set the location of =mmdc= to the right path. Resolution of =~= is not really supported by org-babel. So the absolute path is resolved with ~file-truename~:
#+begin_src emacs-lisp
...
*Note*: It is necessary, that [[https://nodejs.org/][Node.js]] in version 20 or greater or equals than 22 is set up and that the command ~npm i -g <language-server>~ can be executed!
#+begin_src emacs-lisp
